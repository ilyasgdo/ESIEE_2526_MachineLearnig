...
\emph{Interprétation} : \begin{itemize}[left=0pt]
  ...
  \item IoU $\geq$ 0.5 : détection généralement considérée valide
  \item IoU $\geq$ 0.75 : haute précision de localisation
...
\begin{itemize}[left=0pt]
  ...
  \item \emph{mAP@0.50:0.95} (COCO) : moyenne sur IoU $\in$ \{0.50,...,0.95\}, plus exigeant (souvent ~15–20 pts \emph{en dessous} de mAP@0.50 pour un même modèle).
\end{itemize}
...
\end{itemize}
\emph{Variabilité matériel} : GPU A100/V100 $\gg$ RTX 3060 $\gg$ CPU $\gg$ Edge (Jetson/mobile). Optimisations courantes : quantification (FP32$\to$INT8), pruning, distillation, TensorRT/ONNX (gain 2--5$\times$; perte mAP ~1--3 pts).
...
\begin{itemize}[left=0pt]
  \item \textbf{Surveillance sécurité} : rappel élevé, mAP@0.50, FPS $\geq$ 25.
  \item \textbf{Conduite autonome} : mAP@0.50:0.95, FPS $\geq$ 30, latence < 50 ms.
  \item \textbf{Retail analytics} : précision, rappel, F1 élevé ($\leq$ temps réel strict).
  \item \textbf{Inspection industrielle} : précision élevée, IoU/mAP@0.75 (localisation fine).
  \item \textbf{Analyse sportive} : mAP@0.50, FPS $\geq$ 30, F1 (fluidité prioritaire).
...
\begin{table}[h]
  ...
  \begin{tabular}{p{3.2cm} p{4.0cm} p{3.2cm} p{5.6cm}}
    ...
    \midrule
    Surveillance & Rappel, mAP@0.50, FPS & Rappel > 0.95, FPS $\geq$ 25 & Éviter FN critiques, localisation modérée suffisante, temps réel requis. \\
