% Première partie — Résumé exécutif et Introduction
...
En termes de faisabilité, l’entreprise peut déployer une solution efficace en combinant un modèle performant (type YOLO ou Transformer selon usage), des \textit{datasets} publics adaptés (ImageNet VID, YouTube-VOS) et en s’engageant à assurer la conformité avec la réglementation RGPD au travers de mesures de protection des données robustes. Une implantation progressive, avec \textit{proof-of-concept} sur des cas d’usage ciblés, suivie d’itérations d’amélioration, est recommandée pour maîtriser les coûts et les risques techniques.

\section{Introduction et contexte}
\subsection{Mise en contexte du projet}
...
\begin{itemize}[left=0pt]
    \item Explorer exhaustivement les méthodes de Machine Learning existantes et émergentes pour la reconnaissance d’objets vidéo ;
    \item Comparer leurs performances techniques (précision, vitesse, robustesse) et leur applicabilité aux cas d’usage industriels ;
    \item Identifier les défis techniques, biais potentiels dans les données et limites des approches actuelles ;
    \item Formuler des recommandations concrètes orientant le choix des technologies et des \textit{datasets}, ainsi que les modalités de conformité vie privée.
\end{itemize}

\subsection{Méthodologie de recherche}
La recherche bibliographique s’est appuyée sur des bases spécialisées telles que Google Scholar, IEEE Xplore et arXiv pour accéder aux publications scientifiques récentes, ainsi que sur des ressources techniques industrielles actualisées en 2025 (blogs reconnus, documentations GitHub, rapports d’experts). Les méthodes ont été sélectionnées en fonction de leur popularité, leur performance démontrée sur des \textit{benchmarks} standards (ImageNet VID, COCO), et leur pertinence pour les besoins temps réel ou haute précision.
